...
### 実測（2026-09-20, ズーキーパー, goal = レベル 2）

**seed 0〜4、5 局ずつ**

| エージェント | 手数（各 seed） | 平均手数 | レイテンシ中央値 | 1 局のコスト | agreement | mean_regret | fallbacks |
|---|---|---|---|---|---|---|---|
| `jev`（System One） | 16 / 21 / 25 / 18 / 15 | **19.0** | 391〜419 ms | $0.0017〜0.0037 | 0.31〜0.73 | 0.41〜1.43 | 0 |
| `heuristic`（基準線） | 22 / 14 / 24 / 16 / 25 | 20.2 | 0 ms | $0 | 1.00 | 0.00 | 0 |

**seed 7 を 3 者で**

| エージェント | 手数 | レイテンシ中央値 | コスト | agreement | mean_regret | fallbacks |
|---|---|---|---|---|---|---|
| `jev` | 19 | 450 ms | $0.002657 | 0.74 | 0.29 | 0 |
| `heuristic` | 19 | 0 ms | $0 | 1.00 | 0.00 | 0 |
| `operator`（Claude Code / Opus 5） | **18** | 7,635 ms | 計測せず | 0.61 | 0.78 | 0 |

**seed 0 を 3 者で**（上の動画はこの局）

| エージェント | 手数 | レイテンシ中央値 |
|---|---|---|
| `operator` | **14** | 6,635 ms |
| `jev` | 16 | 419 ms |
| `heuristic` | 22 | 0 ms |

読み取れること:

- **jev はヒューリスティックとよく違う手を選ぶ**（一致率 31〜73%）のに、
  平均手数は 19.0 対 20.2 でむしろ速い。評価式の最善手をなぞっているのではなく、
  別の筋で同等以上の判断をしている
- **1 手 400ms 前後・1 局 0.2〜0.4 円**。記事の Jev の数値（239ms）と同じ桁
- `operator` は 1 手あたり 6〜8 秒かかる。判断の質は近いが、
  **ゲームループに乗せるには 15 倍以上遅い** — Jev が効くのはまさにここ

`claude`（Haiku 4.5）はまだ行がない。ゲートウェイの残高がゼロで
`402 billing_error / Insufficient balance` のまま、1 手も API に届いていない。

> `operator` は API ではなく、このリポジトリを書いている Claude Code 自身が
> 1 手ずつ選んだもの。**Haiku 4.5 ではない**ので、記事の Haiku の数字とは
> 比較できない。レイテンシは「答えを書き戻すまでの実時間」で、API の往復時間とは
> 意味が違う。トークンとコストは計測していない。
