...
（[mp4 版・実時間](docs/jev_vs_haiku.mp4) / [静止画](docs/jev_vs_haiku.png)）

**左が jev（System One）、右が claude（Haiku 4.5）。同じ seed・同じ盤面・同じ候補手を配り、
同じゲートウェイ・同じコード経路で、同時に走らせている。**違うのは「どれが最善か」を
誰に聞くかだけ。

上の GIF は 3 倍速だが、[mp4 は実時間](docs/jev_vs_haiku.mp4)。
**jev が 12 手目を打っている時点で、Haiku はまだ 1 手目**。
しかもその時点の累計コストは jev $0.0019 に対し Haiku $0.0023 で、
12 倍打っている jev の方が安い。
...
| エージェント | score 平均 | score 範囲 | レイテンシ / 手 | コスト / 局 | fallbacks |
|---|---|---|---|---|---|
| `jev`（System One） | **2,644** | 2,360〜2,860 | **442 ms** | **$0.0041** | 0 |
| `claude`（Haiku 4.5） | 2,578 | 2,130〜3,120 | 2,583 ms | $0.0453 | 0 |
| `heuristic`（基準線） | 2,552 | 1,890〜3,150 | 0 ms | $0 | 0 |

**jev は Haiku より 5.8 倍速く、11 倍安い。スコアはほぼ同じ。**

スコア差（2,644 / 2,578 / 2,552）は seed ごとの標準偏差（jev 192・Haiku 444）より
小さいので、**判断の質に有意差があるとは言えない**。差が明確に出るのは速度とコストの方。
...
30 手固定とは別に、レベル 2 到達までの手数でも測ってある（seed 0〜2）。
...
**この 2 つの指標は順位が逆になる。** 30 手固定では jev が heuristic をやや下回り、
到達手数では上回る。固定手数はノルマ進行が報われない純粋な点取りになるので、
点取りに最適化された評価式が有利になるため。どちらか一方で優劣は言えない。
...
だが、30 手スコアは 3 者で最下位。`agreement` も同じで、**どちらも「評価式への
