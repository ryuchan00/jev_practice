...
![jev と operator を左右に並べた画面](docs/jev_vs_operator.gif)

（[mp4 版](docs/jev_vs_operator.mp4) / [静止画](docs/jev_vs_operator.png)）

**左が jev（本物の API）、右が operator（Claude Code 自身が 1 手ずつ選ぶ）。**
同じルール・同じ候補手を配り、違うのは「どれが最善か」を誰に聞くかだけ。
盤面の下にレベル・スコア・タイマー・動物ごとのノルマ・使用トークン・累計コスト・
レイテンシ中央値・フォールバック数が並ぶ。

1 手は **入れ替え（青枠）→ 消える 3 匹が黄色く弾ける → 上から落ちてくる** の順に
見せる。どこが動いてどこが消えたのかが分かるように、盤面を差し替える前に
0.34 秒だけ「消える直前」を挟んでいる。

> この動画は 2 本を別々に撮って左右に合成したもので、**両者を同じ尺に伸縮している**。
> 実際の速さは画面内の `latency (median)` を見ること（jev 423ms / operator 6,760ms）。
> jev 単独の録画は [docs/zookeeper.mp4](docs/zookeeper.mp4)。
...
### 実測（2026-09-20, ズーキーパー, goal = レベル 2, seed 0〜2）

| エージェント | 手数（seed 0 / 1 / 2） | 平均手数 | mean_regret 平均 | agreement | レイテンシ中央値 | 1 局のコスト | fallbacks |
|---|---|---|---|---|---|---|---|
| `haiku-sub`（Haiku 4.5, サブエージェント） | 11 / 19 / 18 | **16.0** | 1.16 | 0.22〜0.79 | 9〜16 s | 計測せず | 0 |
| `jev`（System One, API） | 16 / 21 / 25 | 20.7 | 0.82 | 0.31〜0.52 | **391〜419 ms** | **$0.0025〜0.0037** | 0 |
| `heuristic`（基準線） | 22 / 14 / 24 | 20.0 | 0.00 | 1.00 | 0 ms | $0 | 0 |

#### mean_regret は品質の指標ではない

この 3 局でいちばんはっきりしたのはこれ。

**`haiku-sub` は `mean_regret` が最悪（1.16）なのに、手数は最少（16.0）。**

`mean_regret` は「選んだ手とヒューリスティック最善手の**評価値の差**」であって、
「最善からどれだけ離れたか」ではない。評価式そのものが正解ではないので、
そこから離れた手が悪いとは限らない。実際 `heuristic` は定義上 regret 0 だが、
レベル到達は平均 20.0 手で最速ではない。

同じことが `agreement` にも言える。`jev` は 31〜52% しか一致していないのに
平均手数は heuristic とほぼ同じで、`haiku-sub` は一致率が 22〜79% と大きく
振れながら最速。**どちらも「評価式への追従度」であって、独立した品質指標ではない。**

素直に効くのは `progress` に到達するまでの手数の方。ただし——

#### 手数は seed で大きく振れる

`heuristic` ですら 14〜24 手（seed 1 で 14、seed 2 で 24）。
ノルマが全種に課されるので、**ボトルネックの種が盤面に降ってこない間は打つ手がない**。
Haiku のサブエージェントも 2 局とも「パンダが何手も出てこず手数を温存し続けた」
「サルが 15〜17 手目まで現れなかった」と報告している。

**3 seed では順位を決められない。** 上の平均差（16.0 / 20.7 / 20.0）は
seed ごとの振れ幅（±5 手）より小さいので、これをもって優劣は言えない。
seed を増やすのが本筋だが、サブエージェント経由の Haiku は 1 局 3〜5 分かかり、
セッション上限にも当たるため、ここまでで止めている。

#### 現時点で言えること

- **jev は 1 手 400ms、1 局 0.2〜0.4 円で、基準線と同等に打てている**。
  ここは 3 seed でもぶれない（レイテンシとコストは seed 依存が小さい）
- `haiku-sub` は 1 手 9〜16 秒。判断の質は互角以上に見えるが、
  **ゲームループに乗せるには 20 倍以上遅い**
- `claude`（API 経由の Haiku 4.5）はまだ行がない。ゲートウェイの残高がゼロで
  `402 billing_error / Insufficient balance` のまま 1 手も届いていない

> `haiku-sub` は Claude Code のサブエージェントとして動く Haiku 4.5。
> **Messages API 経由ではない**ので、レイテンシにはハーネスとツール往復の
> オーバーヘッドが乗る。記事の Haiku の 1,011ms とは比較できない。
> トークンとコストもこちらでは計測していない。判断の質（手数・agreement・
> mean_regret）は本物の Haiku 4.5 の実測。
